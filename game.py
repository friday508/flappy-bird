...
class Game:
    def __init__(self):
        ...
        self.score = 0
        self.font = pg.font.Font(resource_path("Images/font.ttf"), 24)
        self.score_text = self.font.render("Score: 0 ", True, (0, 0, 0))
        self.flap_sound = pg.mixer.Sound(resource_path("Images/flap.wav"))
        self.score_sound = pg.mixer.Sound(resource_path("Images/score.wav"))
        self.dead_sound = pg.mixer.Sound(resource_path("Images/dead.wav"))
        self.flap_sound.set_volume(0.5)
        self.dead_sound.set_volume(0.5)
        self.score_sound.set_volume(0.5)
        self.has_played_dead_sound = False
    ...
    def checkCollisions(self):
        if len(self.pipes) > 0:
            if self.bird.rect.bottom > 568:
                ...
                if not self.has_played_dead_sound:
                    self.dead_sound.play()
                    self.has_played_dead_sound = True
    ...
    def setUpBgAndGround(self):
        self.bg_img = pg.transform.scale_by(pg.image.load(resource_path("Images/bg.png")).convert(), self.scale_factor)
        self.ground1_img = pg.transform.scale_by(pg.image.load(resource_path("Images/ground.png")).convert(), self.scale_factor)
        self.ground2_img = pg.transform.scale_by(pg.image.load(resource_path("Images/ground.png")).convert(), self.scale_factor)
